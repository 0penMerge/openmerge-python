...
class AsyncOpenMerge:
    """Async API parity with OpenMerge; one pooled client per application lifespan."""

    hosted_widget_url = staticmethod(hosted_widget_url)
    ...
    def __init__(
        ...
        *,
        ...
        self._client = client or httpx.AsyncClient(
            timeout=timeout, limits=httpx.Limits(max_connections=100, max_keepalive_connections=50)
        )
    ...
    async def __aenter__(self) -> AsyncOpenMerge:
        return self
    ...
    async def list_integrations(self, workspace_id: str) -> List[ConnectorIntegration]:
        _require_nonblank("workspace_id", workspace_id)
        value = await self._request("GET", "/integrations", query={"wsid": workspace_id})
        return cast(List[ConnectorIntegration], value["integrations"])

    async def list_models(self, workspace_id: Optional[str] = None) -> List[UnifiedModelDefinition]:
        if workspace_id is not None:
            _require_nonblank("workspace_id", workspace_id)
        value = await self._request("GET", "/models", query={"wsid": workspace_id})
        return cast(List[UnifiedModelDefinition], value["models"])

    async def list_linked_accounts(self, workspace_id: str) -> List[LinkedAccount]:
        _require_nonblank("workspace_id", workspace_id)
        value = await self._request("GET", "/linked-accounts", query={"wsid": workspace_id})
        return cast(List[LinkedAccount], value["linked_accounts"])

    async def get_linked_account(self, linked_account_id: str) -> LinkedAccount:
        _require_nonblank("linked_account_id", linked_account_id)
        value = await self._request("GET", f"/linked-accounts/{quote(linked_account_id, safe='')}")
        return cast(LinkedAccount, value)

    async def pause_linked_account(self, linked_account_id: str) -> AccountTransition:
        _require_nonblank("linked_account_id", linked_account_id)
        value = await self._request(
            "POST", f"/linked-accounts/{quote(linked_account_id, safe='')}/pause"
        )
        return cast(AccountTransition, value)

    async def resume_linked_account(self, linked_account_id: str) -> AccountTransition:
        _require_nonblank("linked_account_id", linked_account_id)
        value = await self._request(
            "POST", f"/linked-accounts/{quote(linked_account_id, safe='')}/resume"
        )
    ...
    async def update_linked_account_schedule(
        self,
        linked_account_id: str,
        cadence_seconds: int,
        *,
    ...
    async def delete_linked_account(self, linked_account_id: str) -> AccountDeletion:
        _require_nonblank("linked_account_id", linked_account_id)
        value = await self._request(
            "DELETE", f"/linked-accounts/{quote(linked_account_id, safe='')}"
        )
        return cast(AccountDeletion, value)
    ...
    async def list_developer_ir_revisions(
        self,
        workspace_id: str,
        oauth_app_id: str,
        model_id: str,
    ...
    async def get_connection_mapping(self, linked_account_id: str) -> ConnectionMappingSchema:
        _require_nonblank("linked_account_id", linked_account_id)
        value = await self._request(
            "GET",
            f"/linked-accounts/{quote(linked_account_id, safe='')}/field-mapping",
    ...
    async def create_connection_mapping_token(
        self,
        linked_account_id: str,
        *,
        host_origin: Optional[str] = None,
    ...
    async def activate_connection_mapping(
        self,
        linked_account_id: str,
        *,
        expected_developer_generations: Mapping[str, int],
    ...
    async def get_connection_mapping_job(self, job_id: str) -> ConnectionMappingJob:
        _require_nonblank("job_id", job_id)
        value = await self._request("GET", f"/connection-mapping-jobs/{quote(job_id, safe='')}")
        return cast(ConnectionMappingJob, value)
    ...
    async def iterate_records(self, model: str, **params: Any) -> AsyncIterator[UnifiedRecord]:
        cursor = params.pop("cursor", None)
        seen = {cursor} if cursor is not None else set()
        while True:
            page = await self.list_records(model, cursor=cursor, **params)
    ...
    async def _submit_record_action(
        self,
        model: str,
        action: str,
        *,
    ...
    async def create_record(
        ...
        *,
        ...
        if not changes:
            raise ConfigurationError("changes must not be empty")
        return await self._submit_record_action(
            model,
            "create",
            workspace_id=workspace_id,
            linked_account_id=linked_account_id,
    ...
    async def get_writeback(self, write_id: str, workspace_id: str) -> Writeback:
        _require_nonblank("write_id", write_id)
        _require_nonblank("workspace_id", workspace_id)
        value = await self._request(
            "GET", f"/unified/writes/{quote(write_id, safe='')}", query={"wsid": workspace_id}
    ...
    async def wait_for_writeback(
        ...
        *,
        ...
        deadline = time.monotonic() + timeout_seconds
        while True:
            write = await self.get_writeback(write_id, workspace_id)
            if write.get("state") in terminal:
                return write
            remaining = deadline - time.monotonic()
            if remaining <= 0:
    ...
    def __enter__(self) -> AsyncOpenMerge:
        raise ConfigurationError("Use async with AsyncOpenMerge")

    def __exit__(self, *_: object) -> None:
        raise ConfigurationError("Use async with AsyncOpenMerge")

    def close(self) -> None:
        raise ConfigurationError("Use await client.aclose()")
